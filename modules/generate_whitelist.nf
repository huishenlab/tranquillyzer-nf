process GENERATE_WHITELIST {
  ...
  output:
  tuple val(sample_id), val(work_dir), val("${work_dir}/transform/${sample_id}/annotation_metadata/${params.whitelist_filename}")
  ...
  tranquillyzer generate-whitelist \\
    ...
    > "${work_dir}/transform/logs/${sample_id}_generate_whitelist.log" 2>&1
  ...
  WL_PATH="${work_dir}/transform/${sample_id}/annotation_metadata/${params.whitelist_filename}"
  if [ ! -f "\${WL_PATH}" ]; then
    echo "ERROR: generate-whitelist did not produce \${WL_PATH}" >&2
    ls -lah "${work_dir}/transform/${sample_id}/annotation_metadata" >&2 || true
    exit 1
  fi
  """
}
